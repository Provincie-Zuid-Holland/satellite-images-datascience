...
from nso_ds_classes.nso_ds_models import cluster_scaler_BH_model
from nso_ds_classes.nso_ds_normalize_scaler import scaler_class_BNDVIH
from nso_ds_classes.nso_ds_normalize_scaler import scaler_class_BH
...
if __name__ == '__main__':
    ...
    for file in glob.glob(run_settings.input_folder_tif_files):
        ...
        a_cluster_annotations_stats_model = cluster_scaler_BH_model(cluster_centers_file)
        ...
        if exists("./scalers/"+path_to_tif_file.split("/")[-1]+"_band3.save") is False:
                ...
                a_nso_cluster_break = nso_ds_cluster.nso_cluster_break(tif_kernel_generator)
                a_nso_cluster_break.make_scalers_pixel_df(output_name = path_to_tif_file.split("/")[-1], parts=2, specific_part=1, multiprocessing= True)
        ...
        a_normalize_scaler_class_BH = scaler_class_BH( "./scalers/"+path_to_tif_file.split("/")[-1]+"_band3.save", scaler_file_band6 = "./scalers/ahn4.save")
           
        tif_kernel_generator.predict_all_output(a_cluster_annotations_stats_model, out_path , parts = run_settings.parts, normalize_scaler= a_normalize_scaler_class_BH )