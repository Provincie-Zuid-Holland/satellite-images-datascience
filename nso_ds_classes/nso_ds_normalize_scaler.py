...
class scaler_class_BNDVIH:
    ...
    def transform(self,pixel_df, col_names = ['band3','band5',"band6"]):
        ...
        pixel_df[col_names[0]] = self.scaler_band3.transform(pixel_df[col_names[0]].values.reshape(-1,1))
        pixel_df[col_names[1]] = self.scaler_band5.transform(pixel_df[col_names[1]].values.reshape(-1, 1))
        pixel_df[col_names[2]] = self.scaler_band6.transform(pixel_df[col_names[2]].values.reshape(-1, 1))
        return pixel_df
