...
if __name__ == '__main__':
    ...
    for file in glob.glob("E:/data/coepelduynen/202[0|1]0[3:9]*ndvi_height.tif"):
        ...
        print(file.replace("\\","/"))
        file = file.replace("\\","/")
        tif_kernel_generator = nso_tif_kernel_iterator.nso_tif_kernel_iterator_generator(file, x_kernel_width , y_kernel_height)

        cloud_percentage = tif_kernel_generator.percentage_cloud(tif_kernel_generator.data)
        if cloud_percentage <= 0.10:
            ...
            print(f"Image contains more than 10% clouds")